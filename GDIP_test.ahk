...
Class GDIP
{
    ...
    __New()
    {
        ; A_PtrSize = size of pointer, in bytes. Either 4 (32-bit) or 8 (64-bit) based on script exe type.
        this.generate_colorName()                               ; Generate color object
        , this.Ptr        := A_PtrSize ? "UPtr"   : "UInt"      ; Set pointer type
        , this.PtrA       := A_PtrSize ? "UPtr*"  : "UInt*"     ; Set pointer address type
        , status          := this.Startup()                     ; Runs GDIP startup and sets token to gdip.token
        , OnExit(this.run_method("__Delete"))                   ; Ensure shutdown runs at script exit
        Return status
    }
